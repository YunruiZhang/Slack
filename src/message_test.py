...
def test_message_send():
	...
	num_mes = len(messages)
	assert(m_id1 == num_mes+1)
...
def test_message_send_except():
	...
	with pytest.raises(InputError):
		assert message_send(owner_token, c_id1, 'long'*250+'l')

	# AccessError:
	#	The authorised user has not joined the channel they are trying to post to
	with pytest.raises(AccessError):
		assert message_send(owner_token, c_id2, 'legal message')
...
def test_message_remove():
	...
	assert(any(m_id1 != m['message_id'] for m in messages))

def test_message_remove_except():
	# InputError:
	#	Message (based on ID) no longer exists
	with pytest.raises(InputError):
		assert message_remove(owner_token, len(messages)+1)

	# AccessError (When none of the following are true):
	#	Message with message_id was sent by the authorised user making this request
	#	The authorised user is an admin or owner of this channel or the slackr
	with pytest.raises(AccessError):
		assert message_remove(member_token, m_id2) # fail

def test_message_edit():
	# Function message_remove(token, message_id, message)
	# Returns {}
	# Given a message, update it's text with new text. 
	message_edit(owner_token, m_id2, new_mes)
	for m in messages:
		if m['message_id'] == m_id2:
			assert(m['message'] == new_mes)
	
	#If the new message is an empty string, the message is deleted.
	message_edit(owner_token, m_id2, '')
	assert(any(m_id2 != m['message_id'] for m in messages))
